...
void convertDotImpl(const LLVMTypeConverter &typeConverter,
                    ...
                    bool opKindIsMXFP4, const DotConversion &op, ArrayRef<ttng::TCGen5CommitOp> commitOps) {
  auto tb = TritonLLVMOpBuilder(loc, rewriter);
...
void convertDot(const LLVMTypeConverter &typeConverter,
                ...
                ttng::TCGen5MMAOp op, ttng::TCGen5MMAOpAdaptor &adaptor, ArrayRef<ttng::TCGen5CommitOp> commitOps) {
...
struct TCGen5MMAOpConversion
  ...
  matchAndRewrite(ttng::TCGen5MMAOp op, OpAdaptor adaptor,
    ...
    convertDot(*getTypeConverter(), rewriter, op.getLoc(), op, adaptor, commitOps);
    rewriter.eraseOp(op);
...
struct TCGen5MMAScaledOpConversion
  ...
  matchAndRewrite(ttng::TCGen5MMAScaledOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    auto commitOps = collectCommitOpAfter(op);
    convertScaledDot(*getTypeConverter(), rewriter, op.getLoc(), op, adaptor, commitOps);
    rewriter.eraseOp(op);
    for (auto commit: commitOps) {
      rewriter.eraseOp(commit);
    }
